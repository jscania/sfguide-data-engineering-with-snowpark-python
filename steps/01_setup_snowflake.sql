...
-- ----------------------------------------------------------------------------
-- Use ULTRASONIC_ROLE for the moment
USE ROLE ULTRASONIC_ROLE;

-- Roles
-- ERROR: Grant not executed: Insufficient privileges.
GRANT EXECUTE TASK ON ACCOUNT TO ROLE ULTRASONIC_ROLE;
GRANT MONITOR EXECUTION ON ACCOUNT TO ROLE ULTRASONIC_ROLE;
GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE ULTRASONIC_ROLE;

-- Databases
CREATE OR REPLACE DATABASE HOL_DB;
GRANT OWNERSHIP ON DATABASE HOL_DB TO ROLE ULTRASONIC_ROLE;

-- Warehouses
CREATE OR REPLACE WAREHOUSE HOL_WH WAREHOUSE_SIZE = XSMALL, AUTO_SUSPEND = 300, AUTO_RESUME= TRUE;
GRANT OWNERSHIP ON WAREHOUSE HOL_WH TO ROLE ULTRASONIC_ROLE;
